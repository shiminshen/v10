...
describe('menu-viewport-transition', () => {
    ...
    const getComputedStyleSpy = vi.spyOn(window, 'getComputedStyle').mockImplementation((element) => {
      ...
      return Object.assign(Object.create(style), {
        ...
        getPropertyValue: (property: string) =>
          property === 'translate' ? '-99.5%' : style.getPropertyValue(property),
  ...
  it('reapplies the viewport transitioning attribute while a child view is active', () => {
    ...
    syncMenuViewport(content, { activeViewId: 'settings-captions-menu' });
  ...
  it('forces root view layout around the active submenu transition', () => {
    ...
    rootView.getBoundingClientRect = vi.fn(() => {
      rootMeasurements.push(!rootView.hasAttribute('data-open'));
