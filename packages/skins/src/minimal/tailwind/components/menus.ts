...
export const menu = {
  root: cn(
    ...
    'box-border min-w-[min(6rem,var(--media-popover-available-width))]',
    'max-w-(--media-popover-available-width) max-h-(--media-popover-available-height)',
    'bg-(--media-popover-background-color) p-1 !overflow-auto overscroll-none rounded-xl',
    'backdrop-filter-(--media-popover-backdrop-filter)',
    'ring-1 ring-(color:--media-popover-border-color) shadow-md shadow-black/10',
    '!transition-[scale,opacity,filter,width,height] duration-(--menu-transition-duration) ease-in-out'
  ),
  ...
  settings: cn(
    'min-w-[min(var(--media-popover-available-width),11rem)] max-h-[min(var(--media-popover-available-height),16rem)]',
